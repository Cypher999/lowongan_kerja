			...
			<div class='card'>
				...
				<div class='card-body'>
					...
					<div class="row">
						...
						<label class='form-control'><?php echo $profil["nm_lengkap"];?></label><br>
						<span class="text-secondary"> Nama lengkap dapat dirubah di opsi "ubah nama" pada menu user, pastikan nama lengkap sesuai dengan nama yg tertera di dokumen, admin berhak melaporkan dan menghapus akun user tanpa peringatan yang diduga melakukan pemalsuan data</span><br>
						<label>Alamat lengkap</label>
						<textarea class="form-control" name='alt_leng' cols='30' rows='3'><?php echo $profil["alt_leng"];?></textarea>
						<label>Provinsi</label>
						<input type='text' class="form-control" name='prov' placeholder="provinsi" required="" value="<?php echo $profil["prov"];?>">
						<label>Kabupaten / Kota</label>
						<input type='text' class="form-control" name='kab' placeholder="kabupaten / kota" required="" value="<?php echo $profil["kab"];?>">
						<label>Kecamatan</label>
						<input type='text' class="form-control" name='kec' placeholder="kecamatan" required="" value="<?php echo $profil["kec"];?>">
						<label>Desa</label>
						<input type='text' class="form-control" name='desa' placeholder="desa" required="" value="<?php echo $profil["desa"];?>">
						<label>Nomor HP</label>
						<input type='text' class="form-control" name='nope' placeholder="no_hp" pattern="[0-9]{1,}" required="" value="<?php echo $profil["no_hp"];?>">
						<label>E-mail</label>
						<input type='text' class="form-control" name='email' placeholder="email" required="" value="<?php echo $profil["email"];?>">
						...
						<div class='row'>
							...
							<div class='col'>
								<input type='text' class="form-control" name='bidang' placeholder="bidang" required="" value="<?php echo $exp[2];?>">
							</div>
							<?php } else{ ?>
						...
						<?php
							...
							?>
						<span class="text-secondary">Pengalaman kerja dan keahlian dibuktikan dengan surat keterangan dari instansi kerja / magang pemilik akun, unggah di bagian "berkas saya"</span>
